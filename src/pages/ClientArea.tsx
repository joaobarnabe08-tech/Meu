    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="text-center">
          <p className="text-slate-500 mb-4">A tua conta ainda não está associada a um perfil de cliente.</p>
          <button onClick={() => signOut()} className="text-emerald-600 font-medium">Sair</button>
        </div>
      </div>
    );
  }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-slate-900 text-lg leading-tight">FitPro</h1>
              <p className="text-xs text-slate-500">Olá, {client.name.split(' ')[0]}</p>
            </div>
          </div>
          <button
            onClick={() => signOut()}
        ...
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4" onClick={() => setVideoModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-900">{videoModal.name}</h3>
              <button onClick={() => setVideoModal(null)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div key={w.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-slate-500">{w.description || 'Sem descrição'}</p>
            </div>
          ...
          <div className="divide-y divide-slate-100">
            {(exercises[w.id] ?? []).map((ex, i) => (
              <div key={ex.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-900 text-sm">{ex.exercise_name}</p>
                ...
                {ex.video_url && (
                  <button onClick={() => onPlay({ url: ex.video_url!, name: ex.exercise_name })} className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-medium hover:bg-emerald-100 transition-colors">
                    <Play className="w-3 h-3" /> Ver
                  </button>
                )}
              </div>
            ))}
            {(exercises[w.id] ?? []).length === 0 && <p className="px-5 py-6 text-center text-sm text-slate-400">Sem exercícios neste treino.</p>}
          </div>
        </div>
      ))}
    </div>
      ...
      {activePlan && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Gorduras', value: `${totalFats}g`, color: 'text-emerald-600', bg: 'bg-emerald-50' },
          ].map(s => (
        ...
        <div key={plan.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-slate-500">{plan.daily_calories ? `${plan.daily_calories} kcal/dia` : 'Calorias não definidas'} · {(meals[plan.id] ?? []).length} refeições</p>
            </div>
          ...
          <div className="divide-y divide-slate-100">
            {(meals[plan.id] ?? []).map(meal => (
              <div key={meal.id} className="flex items-start gap-3 px-5 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-slate-900 text-sm">{meal.meal_name}</h4>
            ...
            {(meals[plan.id] ?? []).length === 0 && <p className="px-5 py-6 text-center text-sm text-slate-400">Sem refeições neste plano.</p>}
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
        <h3 className="font-bold text-slate-900 mb-1">Medidas Corporais</h3>
        <p className="text-sm text-slate-500 mb-4">Última avaliação: {latest.assessment_date}</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {measures.map(m => (
            <div key={m.label} className="p-3 rounded-lg bg-slate-50 border border-slate-100">
              <p className="text-xs text-slate-500 font-medium">{m.label}</p>
              <p className="text-lg font-bold text-slate-900 mt-0.5">{m.value}{m.unit && <span className="text-sm font-normal text-slate-400 ml-1">{m.unit}</span>}</p>
            </div>
          ))}
        </div>
      </div>
      {assessments.length > 1 && (
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-emerald-500" /> Evolução do Peso</h3>
          <div className="space-y-2">
            {[...assessments].reverse().map(a => (
              <div key={a.id} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                <span className="text-sm text-slate-500">{a.assessment_date}</span>
                <span className="font-semibold text-slate-900">{a.weight ? `${a.weight} kg` : '—'}</span>
              </div>
            ))}
          </div>
        </div>
...
function PhotosTab({ clientId }: { clientId: string }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm text-center">
      ...
      <p className="text-slate-500 font-medium">Fotografias de Progresso</p>
      <p className="text-sm text-slate-400 mt-1">As tuas fotografias de progresso aparecerão aqui.</p>
    </div>
  );
}
...
function CalendarTab({ workouts }: { workouts: Workout[] }) {
  ...
  return (
    <div className="space-y-3">
      <h3 className="font-bold text-slate-900">Calendário de Treinos</h3>
      {workouts.map(w => (
        <div key={w.id} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex items-center gap-3">
...
function AssessmentsTab({ assessments, anamnese }: { assessments: PhysicalAssessment[]; anamnese: Anamnese | null }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {anamnese && (
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-3">Anamnese</h3>
          <div className="space-y-3 text-sm">
            {anamnese.goals && <div><p className="text-slate-500 font-medium">Objetivos</p><p className="text-slate-700 mt-0.5">{anamnese.goals}</p></div>}
            {anamnese.health_history && <div><p className="text-slate-500 font-medium">Histórico de Saúde</p><p className="text-slate-700 mt-0.5">{anamnese.health_history}</p></div>}
            {anamnese.notes && <div><p className="text-slate-500 font-medium">Notas</p><p className="text-slate-700 mt-0.5">{anamnese.notes}</p></div>}
          </div>
        </div>
      )}
    </div>
...
function NotificationsTab() {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm text-center">
      ...
      <p className="text-slate-500 font-medium">Sem notificações</p>
      <p className="text-sm text-slate-400 mt-1">As notificações do teu treinador aparecerão aqui.</p>
    </div>
  );
}
