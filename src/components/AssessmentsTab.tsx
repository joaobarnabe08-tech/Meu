...
import { Plus, X, TrendingDown, Activity, Zap, Timer, Heart, Droplets, Gauge, Award, Ruler, HeartPulse, Eye, ChevronDown, ChevronUp, FileDown, Camera, Upload, Trash2 } from 'lucide-react';
import {
...
export default function AssessmentsTab({
  ...
  async function loadPhotos() {
    ...
    if (data) {
      const mapped: ProgressPhoto[] = await Promise.all(data.map(async (f) => {
        const { data: url } = supabase.storage.from('progress-photos').getPublicUrl(`${clientId}/${f.name}`);
        const meta = f.metadata as any;
        return {
          id: f.id,
          url: url.publicUrl,
          category: (f.name.startsWith('front') ? 'front' : f.name.startsWith('side') ? 'side' : 'back') as 'front' | 'side' | 'back',
          taken_at: meta?.created_at || f.created_at || new Date().toISOString(),
          notes: null,
        };
      }));
      setPhotos(mapped);
  ...
  async function uploadPhoto(file: File, category: 'front' | 'side' | 'back') {
    if (!clientId) return;
  ...
  function handleExportPDF() {
    ...
    const data = [
      ...
      { label: 'IMC', value: latest.imc },
      { label: '% Gordura', value: latest.body_fat_percentage, unit: '%' },
      ...
      { label: 'Quadril', value: latest.hip_circumference, unit: 'cm' },
      { label: 'FC Repouso', value: latest.resting_hr, unit: 'bpm' },
      { label: 'FC Max (Tanaka)', value: latest.max_hr_tanaka, unit: 'bpm' },
      { label: 'PA Sistolica', value: latest.systolic_bp, unit: 'mmHg' },
      { label: 'PA Diastolica', value: latest.diastolic_bp, unit: 'mmHg' },
    ];
    exportAssessmentPDF({
        ...
        <div className="flex items-center gap-2">
          {assessments.length > 0 && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-viper-900 text-gold-400 rounded-lg text-sm font-medium hover:bg-viper-800 transition-colors shadow-sm"
            >
      ...
      <div className="bg-white rounded-xl border border-viper-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-viper-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-viper-900">Fotografias de Progresso</h3>
          </div>
          {canEdit && (
              ...
              {(['front', 'side', 'back'] as const).map(cat => (
                <label key={cat} className="cursor-pointer">
                  <input
                    ...
                    accept="image/*"
                    className="hidden"
                    onChange={e => { const f = e.target.files?.[0]; if (f) uploadPhoto(f, cat); e.target.value = ''; }}
                  />
                  <span className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-viper-600 bg-viper-50 hover:bg-gold-50 hover:text-gold-600 rounded-lg border border-viper-200 transition-colors">
                    <Upload className="w-3 h-3" /> {cat === 'front' ? 'Frente' : cat === 'side' ? 'Lateral' : 'Costas'}
                  </span>
                </label>
              ))}
            </div>
        ...
        <div className="p-5">
          {uploadingPhoto && (
            <div className="text-center py-4 text-sm text-viper-400">A carregar fotografia...</div>
          )}
          {photos.length === 0 && !uploadingPhoto ? (
            <div className="text-center py-8">
              <Camera className="w-10 h-10 text-viper-200 mx-auto mb-2" />
              <p className="text-sm text-viper-400">Sem fotografias de progresso.</p>
              {canEdit && <p className="text-xs text-viper-400 mt-1">Carrega fotografias para comparar a evolução ao longo do tempo.</p>}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {(['front', 'side', 'back'] as const).map(cat => {
                ...
                return (
                  <div key={cat}>
                    <p className="text-xs font-semibold text-viper-500 uppercase tracking-wide mb-2">
                      {cat === 'front' ? 'Frente' : cat === 'side' ? 'Lateral' : 'Costas'}
                    </p>
                    <div className="space-y-2">
                      {catPhotos.length === 0 ? (
                        <div className="aspect-[3/4] rounded-lg border-2 border-dashed border-viper-100 flex items-center justify-center">
                          <Camera className="w-6 h-6 text-viper-200" />
                        </div>
                      ) : (
                        catPhotos.slice(0, 3).map(photo => (
                          <div key={photo.id} className="relative group rounded-lg overflow-hidden border border-viper-200">
                            <img src={photo.url} alt={cat} className="w-full aspect-[3/4] object-cover" />
                            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent p-2">
                              <p className="text-[10px] text-white font-medium">
                                {new Date(photo.taken_at).toLocaleDateString('pt-PT')}
                              </p>
                            </div>
                            {canEdit && (
                              <button
                                onClick={() => deletePhoto(photo)}
                                className="absolute top-1.5 right-1.5 w-6 h-6 rounded-lg bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center hover:bg-rose-500"
                              >
