...
export default function ProfileTab({
  ...
  const parqStatus = parqPositiveCount === 0
    ? { label: 'Apto', color: 'text-gold-700', bg: 'bg-gold-50', border: 'border-gold-200', icon: ShieldCheck, iconColor: 'text-gold-600' }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* PAR-Q */}
        <div className="bg-white rounded-xl border border-viper-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-gold-600" />
              <h2 className="font-bold text-viper-900">Questionário PAR-Q</h2>
            </div>
            {form && (
          ...
          <p className="text-xs text-viper-500 mb-5 leading-relaxed">
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-4 p-4 rounded-lg border transition-colors ${
                    value ? 'bg-amber-50 border-amber-200' : 'bg-viper-50 border-viper-100'
                  }`}
                >
                  <span className="shrink-0 w-6 h-6 rounded-full bg-white border border-viper-200 flex items-center justify-center text-xs font-bold text-viper-500 mt-0.5">
                    {i + 1}
                  </span>
                  <p className="flex-1 text-sm text-viper-700 leading-relaxed">{q.label}</p>
                  <div className="flex items-center gap-3 shrink-0 mt-0.5">
                    <label className={`flex items-center gap-1.5 ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'}`}>
                      ...
                      />
                      <span className="text-sm font-medium text-viper-700">Sim</span>
                    </label>
                    <label className={`flex items-center gap-1.5 ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'}`}>
                      <input
                        type="radio"
                      ...
                      />
                      <span className="text-sm font-medium text-viper-700">Não</span>
                    </label>
                  </div>
                </div>
              );
          ...
          {parqPositiveCount > 0 && canEdit && (
            <div className="mt-4">
              <label className="block text-sm font-medium text-viper-700 mb-1.5">
              ...
              <textarea
                ...
                rows={3}
                disabled={!canEdit}
                className="w-full px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 resize-none disabled:bg-viper-100 disabled:cursor-not-allowed"
        ...
        <div className="bg-white rounded-xl border border-viper-200 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="font-bold text-viper-900">Anamnese</h2>
          </div>
          <div className="space-y-5">
            {/* Histórico de Saúde */}
            <div>
              <label className="flex items-center gap-1.5 text-sm font-medium text-viper-700 mb-1.5">
            ...
            {/* Hábitos de Vida */}
            <div className="border-t border-viper-100 pt-5">
              <div className="flex items-center gap-2 mb-4">
                <Activity className="w-4 h-4 text-blue-500" />
                <h3 className="font-semibold text-viper-800">Hábitos de Vida</h3>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Treina? */}
                <div className="sm:col-span-2">
                  <label className="flex items-center gap-1.5 text-sm font-medium text-viper-700 mb-2">
                    <Dumbbell className="w-3.5 h-3.5 text-gold-500" /> Treina atualmente?
                  </label>
                  <div className="flex items-center gap-4 mb-3">
                    <label className={`flex items-center gap-2 ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'}`}>
                      <input
                        ...
                        className="w-4 h-4 accent-gold-500"
                      />
                      <span className="text-sm text-viper-700">Sim</span>
                    </label>
                    <label className={`flex items-center gap-2 ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'}`}>
                      <input
                        type="radio"
                        ...
                        className="w-4 h-4 accent-gold-500"
                      />
                      <span className="text-sm text-viper-700">Não</span>
                    </label>
                  </div>

                  {form?.trains_currently === true && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-4 border-l-2 border-gold-200">
                      <div>
                        <label className="block text-xs font-medium text-viper-600 mb-1">Há quanto tempo treina?</label>
                        <input
                          type="text"
                          value={form?.training_duration ?? ''}
                          onChange={e => canEdit && setForm(f => f ? { ...f, training_duration: e.target.value } : null)}
                          disabled={!canEdit}
                          className="w-full px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 disabled:bg-viper-100 disabled:cursor-not-allowed"
                          placeholder="Ex: 2 anos"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-viper-600 mb-1">Quantas vezes por semana?</label>
                        <input
                          type="text"
                  ...
                  {form?.trains_currently === false && (
                    <div className="pl-4 border-l-2 border-viper-200">
                      <label className="block text-xs font-medium text-viper-600 mb-1">Há quanto tempo não treina?</label>
                      <input
                        type="text"
                        value={form?.training_duration ?? ''}
                        onChange={e => canEdit && setForm(f => f ? { ...f, training_duration: e.target.value } : null)}
                        disabled={!canEdit}
                        className="w-full max-w-xs px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 disabled:bg-viper-100 disabled:cursor-not-allowed"
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-sm font-medium text-viper-700 mb-2">
                    <Cigarette className="w-3.5 h-3.5 text-viper-500" /> Fuma?
                  </label>
                  ...
                  {form?.smokes === true && (
                    <div className="pl-4 border-l-2 border-viper-200">
                      <label className="block text-xs font-medium text-viper-600 mb-1">Cigarros por dia</label>
                      <input
                        type="number"
                        min="0"
                        ...
                        disabled={!canEdit}
                        className="w-24 px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 disabled:bg-viper-100 disabled:cursor-not-allowed"
                        placeholder="0"
                      />
                    </div>
                  )}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="7"
                  />
                  <span className="text-sm text-viper-500 ml-1">horas</span>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={!canEdit}
                    className="w-20 px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 disabled:bg-viper-100 disabled:cursor-not-allowed"
                    placeholder="4"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="2"
                  />
                  <span className="text-sm text-viper-500 ml-1">litros</span>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-viper-100 pt-5">
              <label className="flex items-center gap-1.5 text-sm font-medium text-viper-700 mb-1.5">
                <Target className="w-3.5 h-3.5 text-gold-500" /> Objetivos
              </label>
              <textarea
                value={form?.goals ?? ''}
            ...
            {/* Notas do PT */}
            <div>
              <label className="block text-sm font-medium text-viper-700 mb-1.5">Notas do Personal Trainer</label>
              <textarea
                value={form?.notes ?? ''}
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50 ${
                    saved ? 'bg-gold-100 text-gold-700' : 'bg-gold-500 text-white hover:bg-gold-600'
                  }`}
                >
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-xl border border-viper-200 p-5 shadow-sm">
          <h2 className="font-bold text-viper-900 mb-4">Dados Pessoais</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={label} className="flex items-start justify-between gap-2 py-1.5 border-b border-viper-50 last:border-0">
                <span className="text-xs text-viper-400 font-medium uppercase tracking-wide shrink-0">{label}</span>
                <span className={`text-sm font-medium text-viper-800 text-right ${capitalize ? 'capitalize' : ''}`}>
                  {value || <span className="text-viper-300">—</span>}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl border border-viper-200 p-5 shadow-sm">
          <h2 className="font-bold text-viper-900 mb-3">Notas Gerais</h2>
          <p className="text-sm text-viper-600 leading-relaxed">{client.notes || <span className="text-viper-400">Sem notas registadas.</span>}</p>
        </div>
