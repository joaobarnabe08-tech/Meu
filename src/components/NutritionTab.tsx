    ...
    <div className="grid grid-cols-[1fr_80px] gap-2 items-start">
      <div ref={ref} className="relative">
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-viper-400" />
          <input
            value={entry.query}
            onChange={e => { onChange({ query: e.target.value, selectedFood: null }); setOpen(true); }}
            onFocus={() => setOpen(true)}
            className="w-full pl-8 pr-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400"
            placeholder="Pesquisar alimento..."
          />
        </div>
        {open && filtered.length > 0 && (
          <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-viper-200 rounded-xl shadow-lg overflow-hidden">
            {filtered.map(item => (
              <button key={item.id} type="button" onMouseDown={() => select(item)}
                className="w-full text-left px-3 py-2 hover:bg-gold-50 flex items-center justify-between gap-2 transition-colors border-b border-viper-50 last:border-0">
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-viper-900 truncate block">{item.name}</span>
                  {item.category && <span className="text-xs text-viper-400">{item.category}</span>}
                </div>
                <div className="shrink-0 text-xs text-viper-400 text-right space-y-0.5">
      ...
      <div>
        <input
          ...
          className="w-full px-3 py-2 border border-viper-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/30 focus:border-gold-400 text-center"
          placeholder="g"
        />
      </div>
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-viper-900">Planos Nutricionais</h2>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate('/alimentos')}
            className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-viper-200 text-viper-700 rounded-lg text-sm font-medium hover:bg-viper-50 transition-colors"
          >
      ...
      {activePlan && plans.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Gorduras', value: `${totalFats}g`, unit: `${totalCalories > 0 ? Math.round((totalFats * 9 / totalCalories) * 100) : 0}%`, icon: Droplets, color: 'text-gold-500', bg: 'bg-gold-50' },
          ].map(s => {
            const Icon = s.icon;
            return (
              <div key={s.label} className="bg-white rounded-xl border border-viper-200 p-4 shadow-sm">
                ...
                <p className="text-xs text-viper-400">{s.unit}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="space-y-4">
            ...
            <div key={plan.id} className="bg-white rounded-xl border border-viper-200 shadow-sm overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4">
                <button className="flex items-center gap-3 flex-1 min-w-0 text-left" onClick={() => setExpanded(p => ({ ...p, [plan.id]: !isOpen }))}>
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-viper-900 truncate">{plan.name}</h3>
                    <p className="text-sm text-viper-500">{plan.daily_calories ? `${plan.daily_calories} kcal/dia` : 'Calorias não definidas'} · {plan.meals.length} refeições</p>
                  </div>
                </button>
              ...
              {isOpen && (
                <div className="px-5 pb-5 space-y-3">
                  {plan.meals.map(meal => (
                    <div key={meal.id} className="flex items-start gap-3 p-4 rounded-lg border border-viper-100 bg-viper-50/50 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <h4 className="font-semibold text-viper-900">{meal.meal_name}</h4>
                          {canEdit && (
                            <button onClick={() => onDeleteMeal(plan.id, meal.id)} className="opacity-0 group-hover:opacity-100 w-6 h-6 rounded flex items-center justify-center text-viper-300 hover:text-rose-500 transition-all">
                  ...
                  {addingHere ? (
                    <form onSubmit={e => handleAddMeal(e, plan.id)} className="border border-viper-200 rounded-xl p-4 bg-viper-50 space-y-4">
                      <h4 className="text-sm font-semibold text-viper-700">Adicionar Refeição</h4>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-viper-600 mb-1">Nome da Refeição *</label>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-viper-600 mb-1">Hora *</label>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-medium text-viper-600">Alimentos</label>
                          <div className="grid grid-cols-[1fr_80px] gap-2 w-1/2 sm:w-64 text-xs text-viper-400 pr-7">
                            <span>Nome</span>
                        ...
                        {mealForm.entries.map((entry, i) => (
                          <div key={entry.id} className="flex items-start gap-2">
                            <span className="w-5 text-xs text-viper-400 pt-2.5 text-right shrink-0">{i + 1}.</span>
                            <div className="flex-1 min-w-0">
                              ...
                              {entry.selectedFood && (parseFloat(entry.calories) > 0) && (
                                <div className="flex gap-3 mt-1 pl-1">
                                  ...
                                  <span className="text-xs text-viper-400">P:{parseFloat(entry.proteins).toFixed(1)}g</span>
                                  <span className="text-xs text-viper-400">C:{parseFloat(entry.carbs).toFixed(1)}g</span>
                                  <span className="text-xs text-viper-400">G:{parseFloat(entry.fats).toFixed(1)}g</span>
                                </div>
                              )}
                            </div>
                            <button type="button" onClick={() => removeEntry(entry.id)} disabled={mealForm.entries.length === 1}
                              className="mt-2 w-6 h-6 rounded flex items-center justify-center text-viper-300 hover:text-rose-500 transition-colors disabled:opacity-30 shrink-0">
                              <X className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        ))}
                        ...
                        <button type="button" onClick={addEntry}
                          className="flex items-center gap-1.5 text-xs text-gold-600 hover:text-gold-700 font-medium transition-colors mt-1">
                      ...
                      {formTotals.calories > 0 && (
                        <div className="bg-white rounded-lg border border-viper-200 p-3">
                          <p className="text-xs font-semibold text-viper-500 uppercase tracking-wide mb-2">Total da Refeição</p>
                          <div className="grid grid-cols-4 gap-2 text-center">
                            <div>
                              <p className="text-base font-bold text-rose-500">{Math.round(formTotals.calories)}</p>
                              <p className="text-xs text-viper-400">kcal</p>
                            </div>
                            <div>
                              <p className="text-base font-bold text-blue-600">{formTotals.proteins.toFixed(1)}g</p>
                              <p className="text-xs text-viper-400">proteína</p>
                            </div>
                            <div>
                              <p className="text-base font-bold text-amber-600">{formTotals.carbs.toFixed(1)}g</p>
                              <p className="text-xs text-viper-400">carbs</p>
                            </div>
                            <div>
                              <p className="text-base font-bold text-gold-600">{formTotals.fats.toFixed(1)}g</p>
                              <p className="text-xs text-viper-400">gordura</p>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={() => { setAddingMealTo(null); setMealForm(EMPTY_FORM); }} className="px-3 py-1.5 text-sm text-viper-600 hover:bg-white rounded-lg border border-viper-200 transition-colors">Cancelar</button>
                        <button type="submit" disabled={savingMeal || !mealForm.meal_name.trim()} className="px-3 py-1.5 text-sm bg-gold-500 text-white rounded-lg hover:bg-gold-600 transition-colors disabled:opacity-50">
                    ...
                    <button onClick={() => setAddingMealTo(plan.id)}
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border-2 border-dashed border-viper-200 text-sm text-viper-400 hover:border-amber-300 hover:text-amber-600 transition-colors">
        ...
        {plans.length === 0 && (
          <div className="text-center py-14 bg-white rounded-xl border border-viper-200">
            <UtensilsCrossed className="w-12 h-12 text-viper-200 mx-auto mb-3" />
            <p className="text-viper-500 font-medium">Sem planos nutricionais</p>
            <p className="text-sm text-viper-400 mt-1">Clique em "Novo Plano" para começar.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-viper-100">
              <h2 className="font-bold text-lg text-viper-900">Novo Plano Nutricional</h2>
              <button onClick={() => setShowPlanForm(false)}><X className="w-5 h-5 text-viper-400" /></button>
            </div>
            <form onSubmit={handleAddPlan} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-viper-700 mb-1">Nome do Plano *</label>
              ...
              <div>
                <label className="block text-sm font-medium text-viper-700 mb-1">Calorias Diárias (kcal)</label>
              ...
              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={() => setShowPlanForm(false)} className="px-4 py-2 text-sm text-viper-700 hover:bg-viper-100 rounded-lg transition-colors">Cancelar</button>
                <button type="submit" disabled={savingPlan} className="px-4 py-2 text-sm bg-gold-500 text-white rounded-lg hover:bg-gold-600 transition-colors disabled:opacity-50">
